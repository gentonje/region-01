...
export const ProductCard = ({ imageUrl, productDetail, index, onClick }: ProductCardProps) => {
  ...
  return (
    <div 
      className="w-full bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-md cursor-pointer hover:shadow-lg transition-shadow m-1 p-1 space-y-1 flex flex-col button-glow-outline"
      onClick={onClick}
    >
      <div className="relative w-full overflow-hidden flex justify-center">
        ...
        {productDetail.inStock ? (
          <span className="absolute top-1 right-1 text-xs px-1 py-0.5 rounded-full bg-green-500/90 text-white m-1 button-glow">
            In Stock
          </span>
        ) : (
          <span className="absolute top-1 right-1 text-xs px-1 py-0.5 rounded-full bg-red-500/90 text-white m-1 button-glow-destructive">
            Out of Stock
          </span>
        )}
      </div>
